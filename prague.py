...
def train(trainloader, model, optimizer, criterion, epoch, num_batches, partition_size, sign, lock, List, List_group):
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        #time.sleep(time_comp*_rank*1)
        if _rank == 5:
            time.sleep(time_comp*100)
        '''
        if _rank == 1:
            time.sleep(time_comp*5)
        if _rank == 1:
            time.sleep(time_comp*5)
        '''
    ...
    return train_time, average_time, epoch_loss / num_batches
...
def SSGD(model, _rank, _world_size, partition_size: np.ndarray, sign, lock, List, List_group):
    ...
    while (True):
        if _rank % 2 == 1:
            target = random.choice([i for i in sign.world_list if i%2 == 0])
            sign.target[_rank] = target
            while (True):
                ok=False
                lock.acquire()
                try:
                    if sign.ok[target] == True:
                        ok = True
                        sign.target[target] = _rank
                        #print('active_1', _rank, target, sign.ok[_rank], sign.ok[target])
                        group_reduce = create_group([target, _rank], List, List_group)
                        sign.ok[_rank]= True
                        while(True):
                            if sign.ok[target] == False:
                                sign.ok[_rank] = False
                                #print('active', _rank, target, sign.ok[_rank], sign.ok[target])
                                break
                finally:
                    lock.release()
                if ok == True:
            ...
            sign.ok[_rank]=True
            while(True):
                if sign.ok[sign.target[_rank]] == True and sign.target[sign.target[_rank]]==_rank:
                    #print('positive_1', _rank, sign.target[_rank], sign.ok[_rank], sign.ok[sign.target[_rank]])
                    group_reduce = create_group([sign.target[_rank], _rank], List, List_group)
                    sign.ok[_rank] = False
                    while (True):
                        if sign.ok[sign.target[_rank]] == False:
                            #print('positive', _rank, sign.target[_rank], sign.ok[_rank], sign.ok[sign.target[_rank]])
                            break

                    break
            break

    #print('AllReduce',_rank)
...
if __name__ == "__main__":
    ...
    sign = token_find.select_find()
    manager = torch.multiprocessing.Manager()
    sign.signal = torch.multiprocessing.Value('b', False)
    sign.rank = manager.list([0, 3])
    sign.group = manager.list([])
    sign.target = manager.list([0, 0, 0, 0, 0, 0])
